...
const allListings = async (req, res) => {
  try {
    ...
    res.status(501).json({
      success: false,
      message: "Failed to get all Listings", // Consistent "message" key
    });
  }
};
...
const recentListings = async (req, res) => {
  try {
    // Fetch the top 5 listings sorted by createdAt in descending order
...
<<<<<<< HEAD
export { AddListing, allListings, recentListings, getListById };
=======
...
const getUsedListings = async (req, res)=>{
  try {
    let useListing = await Listing.find({condition:"Used"});
    res.status(200).json({
      data:useListing,
      success:true,
    });
...
try {
  ...
  res.status(200).json({success: true, data: certifiedListings});
} catch (error) {
  console.log(error);
  res.status(404).json({success:false, Message:"Failed to get certified pre-owned listings"});
